    ...
    def list(account_id:, x_moov_version: nil, wallet_id: nil, timeout_ms: nil)
      ...
      if Utils.match_status_code(http_response.status, ['200'])
        if Utils.match_content_type(content_type, 'application/json')
          ...
          obj = Crystalline.unmarshal_json(JSON.parse(response_data), Crystalline::Array.new(Models::Components::Adjustment))
          response = Models::Operations::ListAdjustmentsResponse.new(
            status_code: http_response.status,
            content_type: content_type,
            raw_response: http_response,
            headers: http_response.headers,
            adjustments: T.unsafe(obj)
          )
    ...
    def get(account_id:, adjustment_id:, x_moov_version: nil, timeout_ms: nil)
      ...
      if Utils.match_status_code(http_response.status, ['200'])
        if Utils.match_content_type(content_type, 'application/json')
          ...
          response_data = http_response.env.response_body
          obj = Crystalline.unmarshal_json(JSON.parse(response_data), Models::Components::Adjustment)
          response = Models::Operations::GetAdjustmentResponse.new(
            status_code: http_response.status,
            content_type: content_type,
            raw_response: http_response,
            headers: http_response.headers,
            adjustment: T.unsafe(obj)
          )
